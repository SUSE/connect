...
describe SUSE::Connect::HwInfo::S390 do
  ...
  before :each do
    allow(Open3).to receive(:capture3).with('uname -i').and_return(['s390x', '', success])
  ...
  describe '#uuid' do
    ...
    it 'returns system uuid' do
      expect(Open3).to receive(:capture3).with('read_values -u').and_return(['12345-67890-abcde', '', success])
